  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.views_created > 0 || res.relations_created > 0) {
        toast.success(
          `Importat: ${res.views_created} vistes, ${res.relations_created} relacions`,
        );
      }
...
INSERT INTO Report_ViewRelation (IdView1, IdView2, Relation, ...) VALUES(...);`}
            ...
            {result && (
              <div
                className={`p-3 rounded-lg ${result.errors?.length > 0 && result.views_created === 0 && result.relations_created === 0 ? "bg-destructive/10" : "bg-green-500/10"}`}
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <div className="font-medium">
                      {result.views_created} vistes creades,{" "}
                      {result.relations_created} relacions creades
                    </div>
                    {result.errors?.length > 0 && (
          ...
          >
            <p className="text-sm text-muted-foreground">
              L'últim script importat es guarda automàticament per poder-lo
              recuperar.
            </p>
